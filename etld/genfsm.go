...
func emitChunkFcn(ofile *os.File, chunk int, nstart int, nend int, nodes []*node) {
	fmt.Fprintf(ofile, `
func chunk%d(l int, m int, i int, s int, b byte, pb byte) (nm int, ns int, dobreak bool) {
  switch s {
`, chunk)
	for _, n := range nodes[nstart:nend] {
		if n.b == '*' {
...
func main() {
	...
	nn := len(nodes)
	for idx := 0; idx < nn; idx += chunksize {
		nstart := idx
		nend := idx + chunksize
		if nend >= nn {
			nend = nn - 1
		}
		chunk := idx >> chunkwidth
		emitChunkFcn(ofile, chunk, nstart, nend, nodes)
	}
...
`, chunkwidth)
	...
	for idx := 0; idx < nn; idx += chunksize {
		chunk := idx >> chunkwidth
		fmt.Fprintf(ofile, `		case %d:
		          nm, ns, dobreak := chunk%d(l, m, i, s, b, pb) 
		          m = nm
		          s = ns
		          if dobreak {
		            break loop
